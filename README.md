...
**GTCRN (speech enhancement)** — Lightweight speech denoising / enhancement.
...
This plugin contributes the role(s): `speech-denoise`. A host routes the model by role — it carries no
`targetApps`, so it is not tied to any single application.

**Example host — [Guillotine](https://github.com/HereLiesAz/Guillotine):** Desktop `denoiseModelPath` — `denoise_clip`.

## Model file(s)

- **`gtcrn_simple.onnx`** (role `speech-denoise`) — [upstream](https://huggingface.co/onnx-community/gtcrn/resolve/main/gtcrn_simple.onnx)

Model license: **Apache-2.0 (GTCRN)**. This plugin's manifest/packaging is `Apache-2.0`.
...
```sh
npm install && npm run build     # packs com.hereliesaz.azphalt.gtcrn-1.0.0.azp
